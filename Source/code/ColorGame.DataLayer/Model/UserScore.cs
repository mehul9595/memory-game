        ...
        public static UserScore[] GetTopRankersWithCurrentUser(UserScore currentUserScore)
        {
            ...
            try
            {
                ...
                if (userScores.Count >= 5)
                    userScores.Add(new UserScore() { Email = currentUserScore.Email, Name = currentUserScore.Name, Score = currentUserScore.Score });
                ...
                int currentUserInListCount =
                   filteredRankInfo.Where(r => r.Name == currentUserScore.Name && r.Score == currentUserScore.Score && r.Email == currentUserScore.Email).
                       Count();
                if (currentUserInListCount == 0)
                {
                    UserScore rankInfoData =
                        userScores.Where(r => r.Name == currentUserScore.Name && r.Score == currentUserScore.Score && r.Email == currentUserScore.Email).
                            FirstOrDefault();
